...
- 🔧 **Post-roadmap — dos fixes reportados por uso real.** (1) Las tarjetas de Cola de descargas e Historial usaban un `Grid` con columna `Auto` para los botones al lado del título; ese `Grid` no clampea contra el ancho del contenedor, así que con títulos largos los botones (Pausar/Reanudar/Cancelar, o los 4 íconos de Historial) se desbordaban fuera de la tarjeta — y maximizar la ventana no ayudaba porque el contenido tenía un `MaxWidth` fijo de 760px. Se rediseñaron ambas plantillas para apilar el título arriba y los botones en su propia fila (`WrapPanel`) debajo, con ancho completo disponible, y se subió el `MaxWidth` a 1000px para aprovechar mejor una ventana maximizada. (2) YouTube empezó a devolver `HTTP 403: Forbidden` al descargar streams (el análisis de metadatos seguía funcionando bien) porque yt-dlp, sin un runtime de JavaScript disponible, no puede resolver el desafío de firma que YouTube exige antes de autorizar la descarga, y porque el cliente que yt-dlp elige por defecto (`android_vr`) no es fiable para este caso incluso con el desafío resuelto. Se agregó **Deno** como herramienta empaquetada más (`tools/deno.exe`, MIT, opcional — `YtDlpVideoSource` sigue funcionando en modo degradado si falta) y se fuerza una lista de clientes de respaldo por `--extractor-args` (`web_embedded,android_vr,tv,mweb`) en vez de uno solo, porque cada cliente expone un catálogo de formatos ligeramente distinto y restringir a uno solo hacía que faltaran formatos que antes sí estaban disponibles. Verificado de punta a punta con una descarga real de 291 MB a través de la app (no solo yt-dlp por CLI).
